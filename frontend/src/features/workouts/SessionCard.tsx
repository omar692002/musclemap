...
export function SessionHeroCard({ session }: { session: WorkoutSession }) {
  return (
    ...
    >
      <span
        aria-hidden
        className="pointer-events-none absolute -end-12 -top-16 h-48 w-48 rounded-full bg-surface/15 blur-2xl"
      />
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-surface/20" aria-hidden>
...
export function SessionCard({ session }: { session: WorkoutSession }) {
  return (
    <Link
      to={sessionPath(session.id)}
      className="group flex items-center gap-3.5 rounded-2xl border border-line/80 bg-surface p-3.5 shadow-sm transition hover:border-line-strong hover:shadow active:scale-[0.99]"
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="block font-semibold text-ink">{sessionTitle(session)}</span>
        <span className="block truncate text-sm text-faint">{sessionSubtitle(session)}</span>
      </span>
      <ChevronRight
