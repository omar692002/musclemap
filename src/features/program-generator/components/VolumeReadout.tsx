...
export function VolumeReadout({ volumeByGroup }: VolumeReadoutProps) {
  ...
  return (
    <div className="rounded-2xl border border-line/80 bg-surface p-4 shadow-sm">
      <h3 className="mb-3 text-sm font-bold text-ink">{UiText.weeklyVolume}</h3>
      <ul className="flex flex-col gap-2">
        {rows.map(([group, value]) => (
          <li key={group} className="flex items-center gap-3">
            <span className="w-24 shrink-0 text-xs font-medium text-muted">{MUSCLE_GROUP_LABELS[group]}</span>
