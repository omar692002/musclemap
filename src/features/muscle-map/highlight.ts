...
/**
 * Head-level highlight: headed muscles (delts, pecs, triceps, …) are keyed by
 * the specific head(s) the exercise emphasises — inferred by {@link headsOf} —
 * so only those light up on the head-split 3D model. Non-headed muscles stay
 * keyed by their muscle id. Keys match a mesh's `region.key` (headId ?? muscleId).
 */
